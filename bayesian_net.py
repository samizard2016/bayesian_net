...
class BayesNet:
    def __init__(self,profs: list, statements: list):
        self.source = pd.read_excel("Labor source first 4800.xlsx")      
        self.target = pd.read_excel("labor target 538.xlsx")
        self.profs = profs 
    ...
    def simulate_data(self):
        ...
        new_data = np.zeros(self.target.shape[0]*n_statements).reshape(self.target.shape[0],n_statements)
        for j, statement in enumerate(self.statements):
            vars = self.get_vars(statement)        
            edges = self.build_edges(statement)
            # print(edges)
            train_data = pd.concat([self.source[vars],                         
                self.source.sample(self.target.shape[0],
                                     replace=True,random_state=42)[vars]],axis=0)
            # train_data = self.target[vars]
        ...
        return new_data
    def get_means(self):
        means2024 = self.target[self.statements].values.mean(axis=0)
        means2022 = self.source[self.statements].values.mean(axis=0)
